...
use super::App;
use super::query_utils::{scalar_to_string, string_list_to_csv};
            ...
            "page" => {
                ...
                if let Some(number) = page.get("number") {
                    params.push((
                        ...
                        scalar_to_string(number, "page.number", "build_query_params")?,
                    ));
                }
                if let Some(size) = page.get("size") {
                    params.push((
                        "page[size]".to_string(),
                        scalar_to_string(size, "page.size", "build_query_params")?,
                    ));
                }
            }
            "fields" | "entity_fields" => {
                let joined = string_list_to_csv(value, key, "build_query_params")?;
                if !joined.is_empty() {
                    params.push((key.clone(), joined));
                }
            }
            _ => {
                params.push((key.clone(), scalar_to_string(value, key, "build_query_params")?));
            }
        }
    }
